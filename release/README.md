...
`toris-collection-2.0.1-9.aab` — **広告あり・本番ユニット**の Flutter 版。
**アイコンを直した版**(8 は Flutter の初期ロゴのまま出てしまっていた)。

- versionCode **9** / versionName **2.0.1**
- 8 は 2026-08-21 に製品版で公開済み(Play が配信を確認)。9 はアイコン修正版
...
| 端末のホーム画面 | `toris_app/.../res/mipmap-*`(公開中だった Capacitor 版から移植) | **アプリの更新が要る**(= この 9) |

512 の再生成は `py -3 tools/store_app_icon.py`。**Play は透過を弾く**ので、
ランチャーと同じセージ地(#ecf1e3)に載せてある。
