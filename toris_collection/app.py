...
with tab_help:
    ...
    with st.expander("🔄 基本のサイクル", expanded=True):
        st.markdown("""
    ...
    with st.expander("🐦 鳥に出会えた時", expanded=True):
        st.markdown("""
    ...
    with st.expander("📐 出現確率の仕組み(図鑑の「なぜこの確率?」)", expanded=False):
        st.markdown("""
    ...
    with st.expander("⏳ 不在中ループ", expanded=False):
        st.markdown("""
    ...
    with st.expander("🎁 落とし物のしくみ", expanded=False):
        st.markdown("""
    ...
    with st.expander("🌡️ 土地と気温(月による変化)", expanded=False):
        st.markdown("""
        気温は次の式で決まります。
    ...
    with st.expander("💾 データの所在(この端末にのみ保存)", expanded=True):
        st.markdown("""
    ...
    with st.expander("📺 広告について(すべて任意)", expanded=False):
        st.markdown("""
...
st.markdown(
    ...
    "相互作用データはGloBI(Global Biotic Interactions)の実データを参照して構築したシード"
    "</p>",
    unsafe_allow_html=True
)
