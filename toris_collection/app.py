...
import i18n  # 表示文言の多言語化(EN/JA 切替)
from i18n import t


def _bird_display_name(bird: dict) -> str:
    """表示用の鳥名。英語表示では english、無ければ日本語名にフォールバック。
    (mementos.py の _bird_display_name と同じ考え方を app.py 全体で使い回す)"""
    if i18n.get_lang() == "en" and bird.get("english"):
        return bird["english"]
    return bird.get("name", "")


def _biome_display_name(biome: dict) -> str:
    """表示用のバイオーム名。英語表示では name_en があればそれ、無ければ日本語名。"""
...
def _plant_display_name(plant: dict) -> str:
    """表示用の植物名。英語表示では english、無ければ日本語名。"""
    if i18n.get_lang() == "en" and plant.get("english"):
        return plant["english"]
    return plant.get("name", "")


def _insect_display_name(insect: dict) -> str:
    """表示用の昆虫名。英語表示では english、無ければ日本語名。"""
    if i18n.get_lang() == "en" and insect.get("english"):
        return insect["english"]
...
def render_bird_audio(b_id: str, bird: dict, key_prefix: str = ""):
    ...
    """
    if not XC_AVAILABLE:
        st.caption(t("🔊 {name}の鳴き声 (xeno-canto未読込)", name=_bird_display_name(bird)))
        return
    ...
    cols = st.columns([1, 3])
    with cols[0]:
        if st.button(t("🔊 聴く"), key=f"play_{key_prefix}{b_id}"):
            st.session_state[key] = True

    if st.session_state.get(key):
        with st.spinner(t("{name}の鳴き声を取得中...", name=_bird_display_name(bird))):
        ...
        if audio_bytes:
            ...
            st.caption(t("録音が見つかりませんでした(xeno-cantoに登録なしまたは接続失敗)"))
...
def render_field_view(planted_ids, resident_ids, month, temperature):
    ...
    if not planted_ids and not resident_ids:
        st.info(t("植物を植えて時間を進めると、ここに鳥たちが現れます。"))
        return
    ...
    if plants_to_draw:
        ...
        for i, pid in enumerate(plants_to_draw):
            ...
            name = _plant_display_name(PLANTS[pid])
            # 植物アイコン(絵文字)を大きく描く
            svg.append(
    ...
    for b_id in resident_ids:
        ...
        svg.append(
            ...
            f'{_bird_display_name(bird)}</text>'
        )
        svg.append('</g>')

    # 月・気温の小さなインジケーター(右上)
    label_color = "#f0e8d0" if period == "night" else "#3a3a3a"
    period_label = {"dawn": t("朝"), "day": "", "dusk": t("夕"), "night": t("夜")}[period]
    ...
    svg.append(
        ...
        f'{t("{month}月 {temp}℃", month=month, temp=f"{temperature:.1f}")}{period_text}</text>'
    )
...
def render_login_screen():
    ...
    st.markdown(
        f"<p style='color:#5a7a5a;'>{t('土地を選び、植物を植え、時間が経つのを待つ。やってきた鳥たちの声に耳を澄まそう。')}</p>",
        unsafe_allow_html=True
    )
    st.info(
        t("🔒 進行データ(図鑑・会った日数など)は、この端末のこのブラウザにのみ"
    ...
    if _restore_failed:
        ...
        st.warning(
            t("この端末に保存されていた進行データを自動で読み込めませんでした"
    ...
    if start_mode == "new":
        ...
        pasted_code = st.text_area(
            ...
            height=120,
            placeholder=t("書き出しておいたセーブコードをここに貼り付けてください"),
        )
        ...
        if st.button(t("📥 読み込んで再開"), type="primary", use_container_width=True):
            if not code_to_use:
                st.warning(t("セーブコードを貼り付けるか、ファイルを選んでください。"))
            else:
...
def _handle_ritual_observation():
    ...
    if saved:
        ...
        # 2026-07-12追記(CEO指示): 「庭の様子→鳥に会いに行く」は1回やったら、
        # 滞在中の鳥ぶれ(=入れ替わり)が起きるまで再度は行えないようにする。
        # 同じ滞在中メンバーのまま何度も儀式を繰り返して観察回数・会った日数を
        # 水増しできてしまう(render_ritual側のUIは毎rerunで作り直されるため、
        # 儀式UI自体のsavedフラグだけでは連続再突入を防げない)ことへの対応。
        # ここで「この滞在中メンバー構成には、もう応対済み」を記録し、
        # render_ritual() 側(ritual.py)が滞在メンバーが変わるまで招待自体を
        # 表示しないようにする(観察記録や図鑑・カウントは一切減らさない=
        # 交渉不能の原則2に抵触しない)。
        st.session_state["ritual_done_for_residents"] = frozenset(
            st.session_state.get("residents", [])
        )
...
st.markdown(
    ...
    + t("土地を選び、植物を植え、時間が経つのを待つ。やってきた鳥たちの声に耳を澄まそう。")
    + "</p>",
    unsafe_allow_html=True
)
...
def render_tutorial_banner():
    ...
    biome_name = _biome_display_name(BIOMES.get(st.session_state.biome, {})) or st.session_state.biome
    ...
    with col_next:
        if st.button(content["next_label"], key="tutorial_next_btn", use_container_width=True):
            ...
            st.rerun()
    with col_skip:
        if st.button(t("チュートリアルをスキップ"), key="tutorial_skip_btn", use_container_width=True):
            _tutorial_finish()
            st.rerun()
...
with st.sidebar:
    ...
    st.markdown(t("### 🌍 あなたの土地"))
    biome = BIOMES[st.session_state.biome]
    st.markdown(f"**{_biome_display_name(biome)}**")
    temp_now = current_temperature(st.session_state.biome, st.session_state.month)
    st.markdown(
        f"<div class='stat-box'>"
        f"{t('📅 {month}月', month=st.session_state.month)}<br>"
        f"{t('🌡️ 気温 <b>{temp}℃</b>', temp=f'{temp_now:.1f}')}<br>"
        f"{t('💧 降水量 <b>{precip}mm/年</b>', precip=biome['precip_mean'])}"
        f"</div>",
        unsafe_allow_html=True
    )

    st.markdown("---")
    st.markdown(t("### 📊 コレクション状況"))
    ...
    with st.expander(t("💾 セーブコード(バックアップ)"), expanded=False):
        ...
        if st.checkbox(t("うまくいかない場合はこちら"), value=False, key="save_code_fallback_toggle"):
            st.download_button(
                t("⬇️ セーブコードを書き出す"),
                data=_save_code_str,
                ...
                help=t("アプリ版(サイドロード)では反応しないことがあります。"
            ...
            st.caption(t("⬆️のボタンで反応がない場合は、ここから選択してコピーできます"))
    ...
    try:
        ...
        if cents:
            st.caption(t("🔬 生態的な重要度スコア: {n}種で有効", n=len(cents)))
        else:
            st.caption(t("🔬 生態的な重要度スコア: シードrarityのみ使用"))
    except Exception:
        pass

    if st.button(t("🌬️ ここを離れる"), use_container_width=True,
                 help=t("滞在中の鳥をクリア。植えた植物と図鑑は保持。")):
        st.session_state.residents = set()
        st.session_state.log = []
        st.session_state.absence_events = []
        st.session_state.last_arrivals_info = {}
    ...
    if st.button(t("🔄 セッションをリセット"), use_container_width=True,
                 help=t("この端末の今のセッションを終了し、開始画面に戻ります"
...
if hasattr(st, "dialog"):
        ...
        for f in flash:
            ...
            with cols[1]:
                ...
                if f.get("first"):
                    ...
                    st.caption(t("また会えました。図鑑の観察記録が増えます。"))
        if st.button(t("とじる"), key="obs_dialog_close", use_container_width=True):
            st.rerun()
    ...
    def _welcome_dialog(data):
        ...
        if h >= 48:
            st.caption(t("{n}日ぶりです。留守のあいだに——", n=int(h // 24)))
        elif h >= 2:
            st.caption(t("しばらくぶりです。留守のあいだに——"))
        ...
        if st.button(t("庭を見る"), key="welcome_dialog_close",
...
with tab_home:
    ...
    if _flash:
        ...
        if _first_names:
            st.success(t(
                "🪶 {names} に会えました!図鑑に詳細なアイコンが登録され、ラジオにも加わります。",
                names=_njoin.join(_first_names),
            ))
        if _repeat_names:
            st.success(t(
                "🪶 {names} にまた会えました。図鑑の観察記録が増えます。",
                names=_njoin.join(_repeat_names),
            ))
    ...
    if _ad_flash:
        ...
        if _ad_kind == "item_success":
            st.success(t(
                "🎁 広告を見てくれてありがとう。今日は「{label}」を庭に置きました(6時間)。",
                label=_ad_label,
            ))
        elif _ad_kind == "ad_unavailable":
            st.info(t("この広告リワードは、アプリ版(Android)でのみご利用いただけます。"))
    ...
    if _dist_events:
        ...
        st.markdown(
            ...
            f"margin-bottom:4px;'>{t('🌿 庭の移ろい')}</div>{_dist_rows}</div>",
            unsafe_allow_html=True,
        )
    ...
    if _abs_events:
        ...
        st.markdown(
            ...
            f"{t('🌙 不在中の出来事: {summary}', summary=_summary)}</div></div>",
            unsafe_allow_html=True
        )
        ...
        if False and _new_mems:
            ...
            st.markdown(
                ...
                f"{t('✨ 新しい落とし物が {n} 個', n=len(_new_mems))} "
                f"<span style='font-size:1.4em;'>{''.join(new_icons)}</span>{extra}"
                f"</div></div>",
                unsafe_allow_html=True
            )

        with st.expander(t("出来事を見る"), expanded=False):
            for _ev in _abs_events:
                _bird = BIRDS.get(_ev["bird_id"], {})
                _bird_name = _bird_display_name(_bird) or _ev["bird_id"]
    ...
    if garden_items.is_active(_garden_item):
        ...
        st.markdown(
            ...
            f"{t('{emoji} 今日は「{name}」を置いています(あと{hrs}時間)。', emoji=_gi_meta.get('emoji', '🎁'), name=t(_gi_meta.get('name', '')), hrs=f'{_gi_hours:.1f}')}"
            f"<span style='color:#8a9a8a; font-size:0.85em;'>"
            f"({t(_gi_meta.get('hint', ''))})</span></span></div>",
            unsafe_allow_html=True,
        )
    ...
    with col1:
        ...
        if new_biome != st.session_state.biome:
            st.warning(
                t("⚠️ {biome} に移ると、現在の植物と滞在中の鳥はリセットされます。",
                  biome=_biome_display_name(BIOMES[new_biome])),
                icon="🌬️",
            )
            if st.button(
                t("✓ {biome} に移る", biome=_biome_display_name(BIOMES[new_biome])),
                type="primary", use_container_width=True,
            ):
        ...
        st.markdown(
            ...
            f"{t('🌿 アプリを閉じている間にも、フィールドの生態系は動き続けます。次にここを訪れたとき、新しい鳥が来ているかもしれません。')}"
            "</div>",
            unsafe_allow_html=True
        )

    with col2:
        st.markdown(t("### 🐦 今、ここにいる鳥たち"))
        if not st.session_state.residents:
            st.info(t("まだ鳥は来ていません。植物を植えて、しばらくしてからまた覗いてみましょう。"))
        else:
            ...
            for b_id in sorted(st.session_state.residents, key=lambda b: BIRDS[b]["rarity"]):
                ...
                new_label = f" 🌟 <b>{t('NEW')}</b>" if is_new else ""
...
with tab_radio:
    ...
    if st.session_state.get("current_tester_id"):
        ...
        # 目覚ましは「ラジオの設定のひとつ」という位置づけ(CEO 2026-08-11)。
        # ラジオと同格の機能にはしない。設定を渡すだけで、実際に鳴らすのは
...
# ---------- Tab: Plant ----------
with tab_plant:
    st.markdown(t("### 🌱 植物を植える"))
    st.markdown(t("植物が他の生き物(昆虫・鳥)と**どう相互作用するか**が、やってくる鳥を決めます。"))
    ...
    st.markdown(
        ...
        f"<b>{t('土地の収容力: {now} / {max} 種', now=n_now, max=max_plants)}</b>"
        f"<span style='color:#888; margin-left:10px; font-size:0.88em;'>"
        f"({t('{biome}は最大 {max} 種まで植えられます', biome=_biome_display_name(biome_meta).split('(')[0], max=max_plants)})</span>"
        f"</div>",
        unsafe_allow_html=True,
    )
    if is_full:
        st.warning(
            t("土地が一杯です。新しく植えるには、下の「植えた植物」から1つ抜いてください。"),
            icon="🌾"
        )
    ...
    for i, (pid, plant) in enumerate(available.items()):
        with cols[i % 3]:
            ...
            icon = plant.get("icon", "🌱")
            label = f"{'✅' if is_planted else icon} {_plant_display_name(plant)}"
        ...
        for idx, pid in enumerate(st.session_state.planted):
            ...
            with cols_p[1]:
                ...
                if st.button("🗑️", key=f"remove_{idx}_{pid}",
                             help=t("{plant}を抜く", plant=_plant_display_name(pl)),
                             use_container_width=True):
...
if False:  # with tab_sim:
    ...
    st.caption(
        t("📅 {month}月 · 🌡️ {temp}℃", month=st.session_state.month, temp=f"{sim_temp:.1f}")
    )
    ...
    with col_bird:
        ...
        sim_bird = st.selectbox(
            ...
            format_func=lambda b: f"{_bird_display_name(BIRDS[b])} (★{int(BIRDS[b]['rarity']*5)+1})",
            label_visibility="collapsed",
            key="sim_bird_select",
        )

    with col_plant:
        st.markdown(t("**🌱 追加候補の植物を選ぶ**"))
        ...
        sim_plant = st.selectbox(
            ...
            format_func=lambda p: (
                f"{PLANTS[p].get('icon', '🌱')} {_plant_display_name(PLANTS[p])}"
                + (t(" (植え済み)") if p in already_planted_set else "")
            ),
            label_visibility="collapsed",
    ...
    if sim_bird and sim_plant:
        ...
        if note_already:
            verdict_color = "#888"
            verdict_label = t("(既に植え済み・効果は反映済み)")
        elif delta > 0.001:
        ...
        with st.expander(t("🔍 計算の内訳")):
            st.markdown(t("**{bird} の出現確率の構成**", bird=_bird_display_name(bird)))
            cols_inf = st.columns(2)
            with cols_inf[0]:
                st.markdown(t("**現状(before)**"))
                st.write(t("気温適合度: {v}", v=f"{info_before['temp_fit']:.2f}"))
                st.write(t("バイオーム補正: ×{v}", v=info_before['biome_bonus']))
                st.write(t("食物スコア: {v}", v=f"{info_before['food_score']:.2f}"))
                st.write(t("レア度係数: {v}", v=f"{info_before['rarity_factor']:.2f}"))
            with cols_inf[1]:
                st.markdown(t("**追加後(+{plant})**", plant=_plant_display_name(plant)))
                st.write(t("気温適合度: {v}", v=f"{info_after['temp_fit']:.2f}"))
                st.write(t("バイオーム補正: ×{v}", v=info_after['biome_bonus']))
                st.write(t("食物スコア: {v}", v=f"{info_after['food_score']:.2f}"))
                st.write(t("レア度係数: {v}", v=f"{info_after['rarity_factor']:.2f}"))

            if info_after["incoming_paths"]:
                st.markdown("---")
                st.markdown(t("**追加後の食物経路**"))
                for kind, target_id, weight in info_after["incoming_paths"][:5]:
                    name = target_id
                    if kind == "plant" and target_id in PLANTS:
                        name = _plant_display_name(PLANTS[target_id])
                    elif kind == "insect" and target_id in INSECTS:
                        name = _insect_display_name(INSECTS[target_id])
                    st.write(t("- {name} (寄与 {w})", name=name, w=f"{weight:.2f}"))
...
with tab_birds:
        ...
        for i, (bid, biome) in enumerate(BIOMES.items()):
            ...
            with sum_cols[i]:
                st.markdown(
                    ...
                    f"📍 {_biome_display_name(biome)}</div>"
    ...
    for b_id, bird in sorted_birds:
        ...
        if filter_mode == "found" and not discovered: continue
        if filter_mode == "unfound" and discovered: continue
        ...
        if view_mode_dex == "region":
            ...
            if this_biome and this_biome != current_header_biome:
                ...
                biome_name = _biome_display_name(BIOMES[this_biome])
                st.markdown(
        ...
        if _met_days >= 100:
            _days_label = "　🏅 " + t("会った日数 {d}日・皆勤の友", d=_met_days)
        elif _met_days >= 30:
            _days_label = "　🌿 " + t("会った日数 {d}日・常連", d=_met_days)
        elif _met_days >= 10:
            _days_label = "　🌱 " + t("会った日数 {d}日・おなじみ", d=_met_days)
        elif _met_days >= 1:
            _days_label = "　" + t("会った日数 {d}日", d=_met_days)
        else:
            _days_label = ""
        with st.expander(
            f"{_icon} "
            f"{_bird_display_name(bird) if discovered else '???'} "
            + t("(レア度 {stars})", stars='★' * (1 + int(bird['rarity'] * 5)))
            ...
            if observed:
                ...
                _badge_msg = badges.badge_message(_bird_display_name(bird), _met_days)
                if _badge_msg:
                    st.markdown(
                ...
                if obs_count:
                    st.caption(t("🪶 近くで観察できた回数: {n}回", n=obs_count))
                    ...
                    if bird["eats_plants"]:
                        ...
                        for p in bird["eats_plants"]:
                            if p in PLANTS:
                                ...
                                plant_lines.append(
                                    f"🌱 {_plant_display_name(pl)} _({pl['scientific']}{en2})_"
                                )
                        if plant_lines:
                    ...
                    if bird["eats_insects"]:
                        for i in bird["eats_insects"]:
                            if i in INSECTS:
                                ...
                                st.markdown(
                                    f"  - 🐛 {_insect_display_name(ins)} _({ins['scientific']}{en2})_"
                                )

                st.write(t("**適温域:** {lo}〜{hi}℃", lo=bird['temp_fit'][0], hi=bird['temp_fit'][1]))
                st.write(t("**好む環境:** {biomes}", biomes=', '.join(_biome_display_name(BIOMES[b]) for b in bird['biome_pref'])))

                info = calculate_arrival_probability(
                    b_id, G_now, st.session_state.biome, st.session_state.month
                )

                st.progress(info["probability"], text=t("現状の出現確率: {p}", p=f"{info['probability']:.0%}"))
                with st.expander(t("なぜこの確率?")):
                    st.write(t("- 気温適合度: {v}", v=f"{info['temp_fit']:.2f}"))
                    st.write(t("- バイオーム補正: ×{v}", v=info['biome_bonus']))
                    st.write(t("- 食物スコア: {s} → 係数 {f}", s=f"{info['food_score']:.2f}", f=f"{info['food_factor']:.2f}"))
                    st.write(t("- レア度係数: {v}", v=f"{info['rarity_factor']:.2f}"))
                    if info.get("centrality_used"):
                        st.write(t(
                            "- GloBI補正済PageRank: **{v}** (生態的な重要度スコアを使用)",
                ...
                if _sugg and _sugg["suggestions"]:
                    ...
                    st.caption(t("✓ 食物条件は満たされています。あとは時間とレア度次第。"))
                ...
                if False:
                    ...
                    if feather_mid not in _owned_set_dex:
                        st.caption(t("🪶 まだ {bird} の羽根は手に入っていません。", bird=_bird_display_name(bird)))
                ...
                if _auto_locs:
                    loc_chips = " ".join(
                        ...
                        f"📍 {_biome_display_name(BIOMES[bb]).split('(')[0]}</span>"
                    ...
                    if loc_chips:
                        st.markdown(
                            ...
                            f"{t('これまで出会った土地:')}</span>{loc_chips}</div>",
                            unsafe_allow_html=True
                        )
                ...
                st.info(t("🔭 まだ遠くから気配を感じただけ。近くまで来てくれたら、詳しい生態が記録されます。"))
            else:
                st.write("???")
                st.caption(t("環境を整えて、出会えるのを待ちましょう。"))
...
if False:  # with tab_mementos:
    ...
    for i, (cat_id, cat_label, cat_color) in enumerate(cat_meta):
        ...
        with cols[i]:
            st.markdown(
                ...
                f"font-size:0.95em;'>{t(cat_label)}</div>"
    ...
    view_mode = st.radio(
        t("表示"),
        options=["by_memento", "by_bird"],
        format_func=lambda k: {"by_memento": t("落とし物別"), "by_bird": t("鳥別")}[k],
        horizontal=True,
        label_visibility="collapsed",
    )

    if view_mode == "by_memento":
        # カテゴリ別に折りたたみで表示
        for cat_id, cat_label, cat_color in cat_meta:
            ...
            with st.expander(
                f"{t(cat_label)}　{owned_in_cat} / {len(cat_items)}",
        ...
        if not discovered:
            st.info(t("まだ鳥が来ていません。先にホーム画面で鳥との出会いを待ちましょう。"))
        else:
            # レア度順
            sorted_birds = sorted(
            ...
            for bird_id in sorted_birds:
                ...
                st.markdown(
                    ...
                    f"<div style='flex-grow:1;'>"
                    f"<b style='font-size:1.05em;'>{_bird_display_name(bird)}</b>"
                    f"<span style='color:#888; font-size:0.85em; margin-left:10px;'>"
                    f"{t('{owned} / {total} 種 ({pct}%)', owned=owned_count, total=total_count, pct=f'{pct:.0f}')}</span>"
                    f"</div>"
...
with tab_network:
    ...
    if G_net.number_of_nodes() == 0:
        st.info(t("まだネットワークがありません。植物を植えてみましょう。"))
    else:
        W, H = 1200, 900
        # pos は既にキャッシュから取得済み
        ...
        cols = st.columns(4)
        with cols[0]:
            st.metric(t("🌱 植物"), t("{n}種", n=stats['n_plants']))
        with cols[1]:
            st.metric(t("🐛 昆虫"), t("{n}種", n=stats['n_insects']))
        with cols[2]:
            st.metric(
                t("🐦 来うる鳥"),
                t("{n}種", n=stats['n_birds_active']),
                help=t("食物経路がつながっている鳥の数")
            )
        with cols[3]:
            st.metric(t("🔗 相互作用"), t("{n}本", n=stats['n_edges']))
        ...
        st.markdown(
            ...
            f"<span style='color:#3a5a3a;'><b>●</b> {t('植えた植物(入力)')}</span>"
            f"<span style='color:#2a4a6a;'><b>●</b> {t('来た鳥(出力)')}</span>"
            f"<span style='color:#88a0b8;'>● {t('未訪問の鳥')}</span>"
            f"<span style='color:#c08060;'>● {t('昆虫')}</span>"
            "</div>",
            unsafe_allow_html=True
        )
...
# ---------- Tab: Help (使い方) ----------
with tab_help:
    st.markdown(t("## ❓ Toris Collection の使い方"))

    with st.expander(t("🔄 基本のサイクル"), expanded=True):
        st.markdown(t("""
    ...
    with st.expander(t("📐 出現確率の仕組み(図鑑の「なぜこの確率?」)"), expanded=False):
        st.markdown(t("""
    ...
    with st.expander(t("⏳ 不在中ループ"), expanded=False):
        st.markdown(t("""
    ...
    with st.expander(t("🌡️ 土地と気温(月による変化)"), expanded=False):
        st.markdown(t("""
        気温は次の式で決まります。
    ...
    with st.expander(t("💾 データの所在(この端末にのみ保存)"), expanded=True):
        st.markdown(t("""
        進行データ(バイオーム・植えた植物・図鑑・会った日数・メモなど)は、
    ...
    with st.expander(t("📺 広告について(すべて任意)"), expanded=False):
        st.markdown(t("""
...
st.markdown(
    ...
    f"{t('相互作用データはGloBI(Global Biotic Interactions)の実データを参照して構築したシード')}"
    "</p>",
    unsafe_allow_html=True
)
