...
## 解決(2026-07-13、実機で確認済み)

**根本原因を確定し、修正・実機検証まで完了した。**

CEOの実機(Pixel 9a、USB接続)を使い、Chrome DevTools Protocol経由でWebViewの
DOM状態を直接調査した結果、ログイン/開始画面(新規スタート・セーブコード再開の
選択画面)は実際には正常に描画され、WebSocket接続も`CONNECTED`、
「はじめる」ボタンまで完成していることが判明した。それにもかかわらず画面は
ネイティブスプラッシュのままで、ユーザーは何も見えず・押せない状態だった。

**確定した根本原因**: `_inject_splash_hide()`(ネイティブスプラッシュを消す関数)
が `st.tabs()` 描画後(=既にセッションを開始したユーザーの画面)でしか
呼ばれておらず、初回ユーザーが最初に到達する`render_login_screen()`
(新規スタート/セーブコード再開の選択画面)では一度も呼ばれていなかった。
初回ユーザーは必ずこの選択画面を経由してからでないとタブ群に到達できないため、
「画面はDOM上完成しているのに、それを覆うスプラッシュが永久に消えない」
という詰み状態になっていた。CORS・WebSocket圧縮・User-Agent・Render
コールドスタート・GPUタイルメモリはいずれも無関係だった
(いずれも実機ログ・DevTools調査で否定または無関連と判明)。

**修正**: `toris_collection/app.py` の `render_login_screen()` 内、
`st.stop()` の直前で `_inject_splash_hide()` を呼ぶよう追加
(関数定義自体も呼び出し順の都合で `render_login_screen()` より前に移動)。
commit `f204557`。Renderに再デプロイ済み。

**実機検証結果**: `pm clear` で完全な初回インストール状態を再現し、
アプリを起動 → 約20秒でログイン画面が正常表示・操作可能に → 「はじめる」を
タップ → タブ画面(フィールドの様子・土地選択・バイオーム説明)まで正常に
到達することを実機で確認した。

なお調査過程で `MainActivity.java` に追加した以下の変更は、直接の原因では
なかったが無害なため残置: watchdogの無期限バックオフ化(一時的な遅延から
自動回復できるようにする安全策として有用)、User-Agentの"; wv)"マーカー除去、
`LAYER_TYPE_SOFTWARE`(GPUタイルメモリ警告への対処として試したが効果は
無かった。副作用も無いため残置)。

## 結論(2026-07-13、解決前の調査メモ・参考として保持)
