...
public class BirdAlarmService extends Service {
    ...
    public static final String ACTION_STOP = "com.toriscollection.app.ALARM_STOP";

    @Override
    public int onStartCommand(Intent intent, int flags, int startId) {
        // 2026-08-13 実機で発覚: **止める手段が実質無かった**。通知には
        // "Tap to stop" と書いてあるのに、タップしてもアプリが開くだけで鳴り止まず
        // (contentIntent が MainActivity を指していた)、アプリを閉じても止まらない
        // (フォアグラウンドサービスなので当然)。CEO が端末を再起動する羽目になった。
        // 目覚ましで「止められない」は最悪の壊れ方なので、通知のボタンと本文タップの
        // 両方で止まるようにする。
    ...
    private MediaPlayer makePlayer(int resId) {
        try {
            ...
            // 2026-08-13 実機(Android 16)で発覚: ここは `MediaPlayer.create(this, resId)`
            // で作ってから setAudioAttributes() を呼んでいたが、create() は生成と同時に
            // prepare まで済ませるため、後から属性を変えても反映されない
            // (dumpsys audio で usage=USAGE_UNKNOWN のままだった)。
            // アラームがアラームとして扱われないと、音量の系統も OS の扱いも
            // 「メディア」になる。目覚ましが鳴らない/小さいという最悪の壊れ方を
            // しうるので、属性を渡す方の create() を使う。
