...
PICKS = {
    ...
    # 風: そよ風(松林を抜ける)。★4.9/160件と評価数が桁違いに多い。
    #     2026-08-13 に 715696(松林の light wind, 116s)から差し替え(CEO 指示)。
    #     さらに前の 378725 は落ち葉が転がる音で「ガサガサして気持ち悪い」。
    "wind":   {"id": 181801, "trim": None},   # 65s "breeze.wav"
    # 小川: 岩を回る小さな流れ。前の素材(548373)は 30秒。
    #     ⚠ スペクトルがホワイト寄り(-0.6dB/oct)で、長く聴くと耳につきやすい。
    #     差し替え候補の筆頭(HANDOFF 2026-08-13 §5)。
    ...
    # ── ここから 2026-08-13 追加(CEO 指定)──────────────────────────
    # 湖: 波より穏やかな水際。スペクトルはピンク近傍(-2.5dB/oct)。
    "lake":   {"id": 326097, "trim": None},   # 94s "LakeWaves"
    # 焚き火: 評価 143件。ブラウン寄り(-6.9dB/oct)で低音が厚く、
    #        鳥の声の帯域とぶつかりにくい。下地として相性が良い。
    "fire":   {"id": 40699, "trim": 120},     # 236s -> 116s "campfire.wav"
    # 洞窟の水滴: 評価 388件。ホワイト寄りかつ断続音なので、常時敷く層というより
    #            「たまに鳴る点」。他と重ねて使う想定で音量を控えめにしてある。
